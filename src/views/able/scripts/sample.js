...
function initSpread() {
	...
	// 加载 实例页面 
	/*
    setCellContent();  //单元格实例
    setFormulaContent();  //公式实例
    setConditionalFormatContent(); //格式化显示实例
    setTableContent();   //表实例
    setSparklineContent(); //图表1(表中图)
    setCommentContent(); //批注
    setPictureContent(); //图片
    setDataContent();  //数据实例
    setSlicerContent(); //查询实例 
    addChartContent();  //图表分析
	*/
   ...
   spread.bind(GC.Spread.Sheets.Events.EditChange, function (sender, args) 
   {
		...
		var sheet = args.sheet, row = args.row, col = args.col;
		//只有白色背景(#ffffff)可以编辑,默认背景色为 undefined
		var sColor = sheet.getCell(row, col).backColor();
		if(sColor != '#ffffff' && sColor != undefined ) //row == 0)
		{
			 sheet.endEdit(true);

			 console.log('只有白色背景(#ffffff)可以编辑');
		}
   });
}
...
{
	// openNewWindow();
	window.open ('DBSet.html', '编辑数据库公式', 'height=768, width=1024, top=100,left=200, toolbar=0, menubar=0, scrollbars=0, resizable=0,location=0, status=0')  //写成一行 
	//window.showModalDialog('DBSet.html', '_blank', 'width=768,height=1024,menubar=no,toolbar=no,location=no,directories=no,status=no,scrollbars=0,resizable=0');
}
